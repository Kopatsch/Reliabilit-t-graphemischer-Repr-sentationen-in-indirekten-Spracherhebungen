...
for (word in words){
  ...
  sf_data_phontype_bool$Item.cat.str <- ifelse(sf_data_phontype_bool$gi_star_phontype_bool >= 1.96 , "Hotspot", "Coldspot")
  sf_data_phontype_levenshtein <- sf_data_phontype %>%
    filter(gi_star_phontype_levenshtein < -1.96 | gi_star_phontype_levenshtein > 1.96)
  sf_data_phontype_levenshtein$Item.cat.str <- ifelse(sf_data_phontype_levenshtein$gi_star_phontype_levenshtein >= 1.96 , "Hotspot", "Coldspot")
  #Moran's I
  ...
  sf_data_lextype_bool$Item.cat.str <- ifelse(sf_data_lextype_bool$gi_star_lextype_bool >= 1.96 , "Hotspot", "Coldspot")
  sf_data_lextype_levenshtein <- sf_data_lextype %>%
    filter(gi_star_lextype_levenshtein < -1.96 | gi_star_lextype_levenshtein > 1.96)
  sf_data_lextype_levenshtein$Item.cat.str <- ifelse(sf_data_lextype_levenshtein$gi_star_lextype_levenshtein >= 1.96 , "Hotspot", "Coldspot")
  #Moran's I
  ...
  # Speicherort
  png(str_glue("//wsl.localhost/Ubuntu/home/kopatsch/Masterarbeit/MA/masterarbeit/visual/maps/map_words/{word}.png"), width=6000, height = 3500)
  ...
  grid.arrange(
    ...
    ggplot() + 
      ...
      geom_rect(aes(xmin = 1, xmax = 5, ymin = 2, ymax = 4), 
                fill = "seashell3", color = "white") +
      geom_text(label = str_glue("Getis-Ord Hotspot Analyse\nzu Levenshtein Distanzen"), aes(x = 3, y = 3), size = 20, color= "black") +
      theme_void(),
    ...
    ggplot(sf_data_item_levenshtein) +
      geom_sf(data = europe, fill = "ghostwhite", color = "#887e6a") +
      geom_sf(data = germany, fill = "grey96", color = "#887e6a") +
      geom_sf(aes(color = Item.cat.str, size = 4)) +
      coord_sf(xlim = c(6,10), ylim = c(47.4, 49.8)) +
    ...
    ggplot(sf_data_phontype_levenshtein) +
      geom_sf(data = europe, fill = "ghostwhite", color = "#887e6a") +
      geom_sf(data = germany, fill = "grey96", color = "#887e6a") +
      geom_sf(aes(color = Item.cat.str, size = 4)) +
      coord_sf(xlim = c(6,10), ylim = c(47.4, 49.8)) +
    ...
    ggplot(sf_data_lextype_levenshtein) +
      geom_sf(data = europe, fill = "ghostwhite", color = "#887e6a") +
      geom_sf(data = germany, fill = "grey96", color = "#887e6a") +
      geom_sf(aes(color = Item.cat.str, size = 4)) +
      coord_sf(xlim = c(6,10), ylim = c(47.4, 49.8)) +
