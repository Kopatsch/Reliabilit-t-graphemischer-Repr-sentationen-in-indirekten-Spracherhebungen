...
for (word in words[1]){
  ...
  # Speicherort
  png(str_glue("//wsl.localhost/Ubuntu/home/kopatsch/Masterarbeit/MA/masterarbeit/visual/maps/map_words/{word}2.png"), width=6000, height = 3500)
  ...
  grid.arrange(
    ...
    ggplot() + 
      ...
      geom_rect(aes(xmin = 1, xmax = 5, ymin = 2, ymax = 4), 
                fill = "seashell3", color = "white") +
      geom_text(label = str_glue("Hotspots und Lexemverteilung"), aes(x = 3, y = 3), size = 20, color= "black") +
      theme_void(),
    ggplot() + 
      geom_rect(aes(xmin = 1, xmax = 5, ymin = 1.5, ymax = 4.5), 
                fill = "white", color = "white") + 
    ...
    ggplot(data = df) +
      ...
      new_scale("fill") +
      geom_sf(data = sf_data_item_bool, aes(color = Item.cat.str, fill = Item.cat.str), shape = 24, size = 10) +
      scale_fill_manual(values = c("Coldspot" = "blue", "Hotspot" = "red")) +
      scale_color_manual(values = c("Coldspot" = "black", "Hotspot" = "black"),
                         name = "\n") + 
      coord_sf(xlim = c(6,10), ylim = c(47.4, 49.8)) +
      theme_bw() +
    ...
    ggplot(data = df) +
      geom_sf(data = europe, fill = "ghostwhite", color = "#887e6a") +
      geom_sf(data = germany, fill = "grey96", color = "#887e6a") +
      geom_tile(aes(fill = lextype, alpha = 0.3, x = LONG, y = LAT), colour = "black") +
      scale_fill_manual(values = c(unname(glasbey.colors(32)))) +
      new_scale("fill") +
      geom_sf(data = sf_data_phontype_bool, aes(color = Phontype.cat.str, fill = Phontype.cat.str), shape = 24, size = 10) +
      scale_fill_manual(values = c("Coldspot" = "blue", "Hotspot" = "red")) +
      scale_color_manual(values = c("Coldspot" = "black", "Hotspot" = "black"),
                         name = "\n") + 
      coord_sf(xlim = c(6,10), ylim = c(47.4, 49.8)) +
    ...
    ggplot(data = df) +
      geom_sf(data = europe, fill = "ghostwhite", color = "#887e6a") +
      geom_sf(data = germany, fill = "grey96", color = "#887e6a") +
      geom_tile(aes(x = LONG, y = LAT, fill = lextype, alpha = 0.1), colour = "black") +
      scale_fill_manual(values = c(unname(glasbey.colors(32)))) +
      new_scale("fill") +
      geom_sf(data = sf_data_lextype_bool, aes(color = Lextype.cat.str, fill = Lextype.cat.str), shape = 24, size = 10) +
      scale_fill_manual(values = c("Coldspot" = "blue", "Hotspot" = "red")) +
      scale_color_manual(values = c("Coldspot" = "black", "Hotspot" = "black"),
                         name = "\n") + 
      coord_sf(xlim = c(6,10), ylim = c(47.4, 49.8)) +
      ...
      theme(legend.position = "none",
            ...
            axis.ticks.x = element_blank(),
            axis.text.y = element_text(size = 0.1, colour = "white"),
            axis.ticks.y = element_blank()),
    ...
    widths=c(1,1,1,0.8,1),
    layout_matrix = cbind(c(1,6,11,16), c(2,7,12,17), c(3,8,13,18), c(4,9,14,19), c(5,10,15,20))
  )
